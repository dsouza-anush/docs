...
- **Missing `max_tokens`**: Always set `max_tokens` to control response length and costs
- **Wrong model ID**: Use the exact model ID (e.g., `claude-4-5-sonnet`, not `claude-4.5-sonnet`)
- **Forgetting to stream**: For chat UIs, always enable `stream: true` to avoid timeout errors and improve UX
- **High temperature for factual tasks**: Use `temperature: 0` for deterministic, factual outputs
- **Not handling rate limits**: Implement exponential backoff for 429 errors
</Warning>
