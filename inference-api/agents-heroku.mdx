...
The `/v1/agents/heroku` endpoint allows you to interact with an agentic system powered by large language models (LLMs) that can autonomously invoke tools based on your messages. Unlike [`/chat/completions`](https://devcenter.heroku.com/articles/heroku-inference-api-v1-chat-completions), which generates a single model response, the `v1/agents/heroku` endpoint supports automatic tool execution and multistep workflows.
