...
- **Mismatched dimensions**: Your `VECTOR(n)` column must match your model's output (Cohere = 1024 dimensions)
- **Wrong `input_type`**: Use `search_document` when indexing, `search_query` when searching - mixing them hurts quality
- **Not batching**: Always batch up to 96 strings per request to reduce costs and latency
- **Text too long**: Keep inputs under 512 tokens for best results; chunk longer documents first
- **Forgetting normalization**: Lowercase and trim whitespace before embedding for consistent results
</Warning>
