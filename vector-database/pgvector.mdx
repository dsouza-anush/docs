...
description: "Store and query vector embeddings in Postgres for semantic search, RAG, and similarity matching"
---

## What are Vector Embeddings?

**Vectors** are lists of numbers that represent the meaning of text, images, or other data. When you convert text to a vector (called an "embedding"), similar concepts end up with similar number patterns.

**Example**: The sentences "I love dogs" and "I adore puppies" would have very similar vectors, even though they use different words.

<Note>
**Why 1536 dimensions?** Different embedding models output different vector sizes. Cohere Embed Multilingual outputs 1024-dimensional vectors. OpenAI's models use 1536. Always match your `VECTOR(n)` column size to your embedding model's output dimension.
</Note>
...
- [ ] A Heroku Postgres database (Essential-0 or higher with PostgreSQL 14+, or Standard-tier with PostgreSQL 15+)
- [ ] A Heroku app with the Managed Inference add-on (for generating embeddings)
- [ ] Basic SQL knowledge

<Warning>
**Not sure what dimension to use?** Check your embedding model's documentation. For Heroku's Cohere Embed Multilingual, use `VECTOR(1024)`.
</Warning>

The pgvector extension for Heroku Postgres adds support for vector data types and similarity search operations. Use it to build semantic search, recommendation systems, and retrieval-augmented generation (RAG) applications.
...
```bash
heroku pg:psql DATABASE_URL -a my-app
```
...
INSERT INTO documents (content, embedding) VALUES
  ('AI helps developers build applications', '[0.1, 0.2, 0.3, ...]'),
  ('Postgres is a powerful database', '[0.4, 0.5, 0.6, ...]');
```

### 4. Search by Similarity

```sql
-- Find 5 most similar documents using cosine distance
SELECT content, embedding <=> '[0.15, 0.25, 0.35, ...]' AS distance
FROM documents
ORDER BY distance
LIMIT 5;
```
...
<Note>
  Build the index **after** loading your data for better recall. See the [Performance](#performance) section for details.
</Note>
...
Here's a full example combining the Heroku Inference API with pgvector for retrieval-augmented generation:
...
# Initialize OpenAI client pointing to Heroku Inference API
client = OpenAI(
...
query = "How do I scale my Heroku app?"
...
# 3. Use retrieved context with chat completion
chat_client = OpenAI(
...
// 1. Generate embedding for user query
const query = "How do I scale my Heroku app?";
...
const relevantDocs = result.rows.map(row => row.content);
...
<CardGroup cols={2}>
  ...
  <Card title="Heroku Tools" href="/tool-use/heroku-tools" icon="wrench">
    Query vectors with pg_vector_query tool
  </Card>
  <Card title="AI Integrations" href="/ai-integrations/llamaindex" icon="puzzle-piece">
    Use with LlamaIndex and other frameworks
  </Card>
  <Card title="Blog Post" href="https://blog.heroku.com/pgvector-for-similarity-search-on-heroku-postgres" icon="newspaper">
    Full tutorial on similarity search
  </Card>
</CardGroup>
